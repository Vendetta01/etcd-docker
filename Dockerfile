...
RUN curl -L ${DOCKER_DOWNLOAD_URL}/${DOCKER_ETCD_VER}/etcd-${DOCKER_ETCD_VER}-linux-amd64.tar.gz -o /tmp/etcd-${DOCKER_ETCD_VER}-linux-amd64.tar.gz
RUN mkdir -p ${DOCKER_INSTALL_DIRECTORY}
RUN tar xzvf /tmp/etcd-${DOCKER_ETCD_VER}-linux-amd64.tar.gz -C ${DOCKER_INSTALL_DIRECTORY}/ --strip-components=1
RUN rm -f /tmp/etcd-${DOCKER_ETCD_VER}-linux-amd64.tar.gz
